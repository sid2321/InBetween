...
it('single jsx file and multiple pdf formats', () => {
    ...
    cy.outDirectoryFileName('DC_GENERATE_26_Update')
    ...
    cy.get(jaDashboard.panel).within(() => {
        ...
        cy.contains(' [PDFX4 2008] ').click({force:true});
        cy.contains(' [High Quality Print] ').click({force:true})
    })
    cy.saveJob();
    cy.get(jaDashboard.jobSection).within(() => {
        cy.get('#scheduler_0').find('.column-output').within(() => {
            cy.get('div').contains('DC_GENERATE_26_Update')
        })
