...
it('change data xml and generate', () => {
    ...
    cy.outDirectoryFileName('DC_GENERATE_46_Update')
    ...
    cy.get(jaDashboard.xmlFile).selectFile(['SimpleData_EN_DE_Test.xml'],{force:true})
    cy.get('@uGroup').find('mat-select').eq(2).within(() => {
        cy.contains('expand_more').click({force:true});
    })
    cy.wait(1000)
    cy.get(jaDashboard.panel).within(() => {
        cy.contains('SimpleData_EN_DE_Test.xml').click();
    })
    ...
    cy.get(jaDashboard.jobSection).within(() => {
        cy.get('#scheduler_0').find('.column-output').within(() => {
            cy.get('div').contains('DC_GENERATE_46_Update')
        })
    ...
    cy.outDirectoryFileName('DC_GENERATE_51_Update')
    ...
    cy.get(jaDashboard.jobSection).within(() => {
        cy.get('#scheduler_0').find('.column-output').eq(0).within(() => {
            cy.get('div').eq(0).contains('DC_GENERATE_51_Update')
        })
        cy.get('mat-checkbox').eq(0).find('[type="checkbox"]').click({force:true});
    })
    cy.get(jaDashboard.localUpdate).click({force:true})
