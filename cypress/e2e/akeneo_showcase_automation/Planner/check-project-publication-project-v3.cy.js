...
describe('check test cases from creating new publication and section v3', () => {
    ...
    it('check publication data', () => {
        ...
        cy.get(plannerSelector.projecSelPlannerpanel).within(() =>{
            cy.contains(' IB_Akeneo_Showcase_V3 ').click({force:true})
        })
        ...
        cy.get(plannerSelector.searchPublication).as('search').type('Akeneo_Products')
        cy.get('app-table').within(() => {
            cy.get('mat-row').should('have.length',1)
        })
        cy.get('@search').clear().type('Akeneo')
        cy.get('app-table').within(() => {
            cy.get('mat-row').as('elements').should('have.length',1)
                ...
                for(let i=0; i<row.length; i++){
                    ...
                    .eq(0).within(() =>{
                        ...
                        .invoke('text').then(text => {
                            console.log(text)
                            let result = text.includes('Akeneo_Products')
    ...
    it('check for new publication', () => {
        ...
        cy.get(plannerSelector.searchPublication).as('search').type('Akeneo_Products_Demo')
        ...
        cy.get('#userName_Akeneo_Products_Demo').find('svg').click({force:true})
        cy.get('.mat-menu-panel').within(() => {
            cy.contains('InBetween Project Manager').click({force:true})
        })

        cy.get('#editDisabled_Akeneo_Products_Demo').within(() => {
            cy.get('span').eq(1).find('img').click({force:true})
        })
        cy.get('#workflowStageText_Akeneo_Products_Demo').should('have.text',' Distribution ')
        cy.get('#userName_Akeneo_Products_Demo').find('span').eq(0).should('have.text',' Assignee ')
    ...
    it('check publication preview', () => {
        ...
        cy.get(plannerSelector.searchPublication).as('search').type('Akeneo_Products_Demo')
        cy.get('#titleSpan_Akeneo_Products_Demo').should('be.visible')
        cy.wait(2000)
        cy.get('#expandButton_Akeneo_Products_Demo').click({ force:true })
        cy.get('#detail_row_Akeneo_Products_Demo').as('section_detail')
        ...
        cy.get('#detail_row_Akeneo_Products_Demo').compareSnapshot('check preview v3 showcase',0.2,retryOptions)
    })
})