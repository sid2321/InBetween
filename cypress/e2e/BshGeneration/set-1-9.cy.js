...
describe('bsh generation BSH_Pricelist_Neff', () => {
    ...
    it('BSH_Pricelist_Gaggenau Generation', () => {

        let renameFileName = "BSH_Pricelist_Gaggenau_Generation.pdf";
        cy.visit(`${userData.login_url}/#/PublicationWizard/home`)
        cy.createNewPublication(projectData.bsh.projectGaggeanau,projectData.bsh.masterPublication,'Test_Performance_S')
        cy.wait(5000);
        ...
        cy.get('.mat-select-panel').within(() => {
            cy.contains('IB_de-DE_A15.xml').click({force: true });
            cy.wait(2000);
        })
        cy.get(elementSelectionSelectors.sideToolBar).within(() => {
            cy.contains('B').click();
