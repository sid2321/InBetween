...
describe('bsh generation BSH_Pricelist_Siemens', () => {
    ...
    it('BSH_Pricelist_Siemens Generation', () => {

        let renameFileName = "BSH_Pricelist_Siemens_Generation.pdf";
        cy.visit(`${userData.login_url}/#/PublicationWizard/home`)
        cy.createNewPublication(' BSH_Pricelist_Siemens ',projectData.bsh.masterPublication,'Test_Performance_S')
        cy.wait(5000);
        ...
        cy.get('.mat-select-panel').within(() => {
            cy.contains('IB_de-DE_A02.xml').click({force: true });
            cy.wait(2000);
        })
        cy.get(elementSelectionSelectors.sideToolBar).within(() => {
            cy.contains('B').click();
